...
//以上就是比较常用的几种数组去重方式，当然，我们最推崇的就是用hash，因为它快，但是我们会发现有时候就不好使了，比如[1,'1'],去重之后就变成了[1],这是为什么呢？其实我们在使用hash的时候就是把数组元素作为hash的key值，那么在使用的过程就会把数组元素变成了字符串，所以成了上面结果，再扩展一下，对于Boolean、null等都会有上面的情况，所以我们不能只关注数组元素的值，还要关注它的数据类型。关于数据类型的判断,常见的typeof, instanceof。这里我推荐使用Object.prototype.toString.call。因此，对于key值就不能是简单的数组元素，而是应该包含该元素的数据类型，如下
Array.prototype.unique = function(){
    var json = {}, newArr = [], len = this.length;
    for(var i = 0; i < len; i++){
